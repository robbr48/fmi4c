...
/* -----------------------------------------------------------------
 ...
 xcor is a vector (type FMIC_N_Vector) which holds the scaled,
 preconditioned correction to the initial guess.

 yg is a length (l_max+1) array of realtype used to hold "short"
 vectors (e.g. y and g).

 vtemp is a vector (type FMIC_N_Vector) used as temporary vector
...
/*----------------------------------------------------------------
 ...
 FMIC_N_Vector used as a template to create new vectors by duplication.
 This routine returns NULL if there is a memory request failure.
 ---------------------------------------------------------------*/

SUNDIALS_EXPORT SpfgmrMem SpfgmrMalloc(int l_max, FMIC_N_Vector vec_tmpl);
...
/*----------------------------------------------------------------
 ...
 FMIC_N_Vector upon exit with return value SPFGMR_SUCCESS or
 SPFGMR_RES_REDUCED. For all other return values, the output x
 is undefined.

 b is the right hand side FMIC_N_Vector. It is undisturbed by this
 function.
 ...
 s1 is an FMIC_N_Vector of positive scale factors for b. (Not 
 tested for positivity.)  Pass NULL if no scaling on b is 
 required.

 s2 is an FMIC_N_Vector of positive scale factors for P x, where
...
SUNDIALS_EXPORT int SpfgmrSolve(SpfgmrMem mem, void *A_data, FMIC_N_Vector x, 
				FMIC_N_Vector b, int pretype, int gstype, 
				realtype delta, int max_restarts, 
				int maxit, void *P_data, FMIC_N_Vector s1, 
				FMIC_N_Vector s2, ATimesFn atimes, PSolveFn psolve, 
