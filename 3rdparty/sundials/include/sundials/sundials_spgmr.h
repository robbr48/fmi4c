...
SUNDIALS_EXPORT SpgmrMem SpgmrMalloc(int l_max, FMIC_N_Vector vec_tmpl);
 ...
 * FMIC_N_Vector upon exit with return value SPGMR_SUCCESS or
 ...
 *
 * s1 is an FMIC_N_Vector of positive scale factors for P1-inv b, where
 * P1 is the left preconditioner. (Not tested for positivity.)
 * Pass NULL if no scaling on P1-inv b is required.
 *
 * s2 is an FMIC_N_Vector of positive scale factors for P2 x, where
...
SUNDIALS_EXPORT int SpgmrSolve(SpgmrMem mem, void *A_data, FMIC_N_Vector x, FMIC_N_Vector b,
			       int pretype, int gstype, realtype delta, 
			       int max_restarts, void *P_data, FMIC_N_Vector s1, 
