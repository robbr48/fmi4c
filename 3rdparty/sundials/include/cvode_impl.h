...
typedef struct CVodeMemRec {
  ...
  FMIC_N_Vector cv_Vabstol;       /* vector absolute tolerance                     */
  ...
  FMIC_N_Vector cv_zn[L_MAX];  /* Nordsieck array, of size N x (q+1).
  ...
  FMIC_N_Vector cv_ewt;     /* error weight vector                                 */
  FMIC_N_Vector cv_y;       /* y is used as temporary storage by the solver
                          The memory is provided by the user to CVode
                          where the vector is named yout.                     */
  FMIC_N_Vector cv_acor;    /* In the context of the solution of the nonlinear
                          equation, acor = y_n(m) - y_n(0). On return, 
                          this vector is scaled to give the est. local err.   */
  FMIC_N_Vector cv_tempv;   /* temporary storage vector                            */
  FMIC_N_Vector cv_ftemp;   /* temporary storage vector                            */
  ...
  long int cv_lrw1;        /* no. of realtype words in 1 FMIC_N_Vector             */
  long int cv_liw1;        /* no. of integer words in 1 FMIC_N_Vector              */
  ...
  int (*cv_lsetup)(struct CVodeMemRec *cv_mem, int convfail, FMIC_N_Vector ypred,
           FMIC_N_Vector fpred, booleantype *jcurPtr, FMIC_N_Vector vtemp1,
           FMIC_N_Vector vtemp2, FMIC_N_Vector vtemp3);

  int (*cv_lsolve)(struct CVodeMemRec *cv_mem, FMIC_N_Vector b, FMIC_N_Vector weight,
           FMIC_N_Vector ycur, FMIC_N_Vector fcur);
...
/*
 * -----------------------------------------------------------------
 * int (*cv_lsetup)(CVodeMem cv_mem, int convfail, FMIC_N_Vector ypred,
 *                 FMIC_N_Vector fpred, booleantype *jcurPtr,
 *                 FMIC_N_Vector vtemp1, FMIC_N_Vector vtemp2,
 *                 FMIC_N_Vector vtemp3);
 * -----------------------------------------------------------------
 ...
 *
 * vtemp1 - temporary FMIC_N_Vector provided for use by cv_lsetup.
 *
 * vtemp3 - temporary FMIC_N_Vector provided for use by cv_lsetup.
 *
 * vtemp3 - temporary FMIC_N_Vector provided for use by cv_lsetup.
 *
...
/*
 * -----------------------------------------------------------------
 * int (*cv_lsolve)(CVodeMem cv_mem, FMIC_N_Vector b, FMIC_N_Vector weight,
 *                  FMIC_N_Vector ycur, FMIC_N_Vector fcur);
 * -----------------------------------------------------------------
 ...
 * fcur contains the FMIC_N_Vector f(tn,ycur). The solution is to be
