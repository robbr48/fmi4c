 ...
 *     (2) using block-based access macros:
 *         get pointer to block ib,jb:
 *            block = BTRIDIAG_BLOCK(A,ib,jb)
 *         then access element bi,bj within block:
 *            BTRIDIAG_BLOCK_ELEM(A,block, bi, bj) = value;
 *     (3) access memory structure directly:
 *         get pointers to all blocks in block-row ib:
 *            lower_block = A->cols[0] + ib*blocksize*blocksize
 *            main_block  = A->cols[1] + ib*blocksize*blocksize
 *            upper_block = A->cols[2] + ib*blocksize*blocksize
 *         values within blocks are stored column-based, to access
 *         element bi,bj use:
 *            lower_block[bi + bj*blocksize] = value;
 *
 * user_data is a pointer to user data - the same as the user_data
 *           parameter passed to CVodeSetFdata.
 *
...
SUNDIALS_EXPORT int CVDlsSetBTridiagJacFn(void *cvode_mem, CVDlsBTridiagJacFn jac);
