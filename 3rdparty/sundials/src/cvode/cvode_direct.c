 ...
 * the macro DENSE_COL and this pointer is associated with an FMIC_N_Vector
 * using the FMIC_N_VGetArrayPointer/FMIC_N_VSetArrayPointer functions. 
 * Finally, the actual computation of the jth column of the Jacobian is 
 * done with a call to FMIC_N_VLinearSum.
 * -----------------------------------------------------------------
 */ 

int cvDlsDenseDQJac(long int N, realtype t,
                    FMIC_N_Vector y, FMIC_N_Vector fy, 
                    DlsMat Jac, void *data,
                    FMIC_N_Vector tmp1, FMIC_N_Vector tmp2, FMIC_N_Vector tmp3)
{
  realtype fnorm, minInc, inc, inc_inv, yjsaved, srur;
  realtype *tmp2_data, *y_data, *ewt_data;
  FMIC_N_Vector ftemp, jthCol;
  long int j;
  int retval = 0;
  ...
  tmp2_data = FMIC_N_VGetArrayPointer(tmp2);
  ...
  ewt_data = FMIC_N_VGetArrayPointer(ewt);
  y_data   = FMIC_N_VGetArrayPointer(y);

  /* Set minimum increment based on uround and norm of f */
  srur = SUNRsqrt(uround);
  fnorm = FMIC_N_VWrmsNorm(fy, ewt);
  minInc = (fnorm != ZERO) ?
  ...
  for (j = 0; j < N; j++) {
    ...
    FMIC_N_VSetArrayPointer(DENSE_COL(Jac,j), jthCol);
    ...
    inc_inv = ONE/inc;
    FMIC_N_VLinearSum(inc_inv, ftemp, -inc_inv, fy, jthCol);

    DENSE_COL(Jac,j) = FMIC_N_VGetArrayPointer(jthCol);
  }

  /* Restore original array pointer in tmp2 */
  FMIC_N_VSetArrayPointer(tmp2_data, tmp2);
...
int cvDlsBandDQJac(long int N, long int mupper, long int mlower,
                   realtype t, FMIC_N_Vector y, FMIC_N_Vector fy, 
                   DlsMat Jac, void *data,
                   FMIC_N_Vector tmp1, FMIC_N_Vector tmp2, FMIC_N_Vector tmp3)
{
  FMIC_N_Vector ftemp, ytemp;
  ...
  ewt_data   = FMIC_N_VGetArrayPointer(ewt);
  fy_data    = FMIC_N_VGetArrayPointer(fy);
  ftemp_data = FMIC_N_VGetArrayPointer(ftemp);
  y_data     = FMIC_N_VGetArrayPointer(y);
  ytemp_data = FMIC_N_VGetArrayPointer(ytemp);

  /* Load ytemp with y = predicted y vector */
  FMIC_N_VScale(ONE, y, ytemp);
