...
#ifdef SUNDIALS_DUMP_ITER_STATS
#include <nvector/nvector_serial.h>
#endif // SUNDIALS_DUMP_ITER_STATS
...
static booleantype cvCheckNvector(FMIC_N_Vector tmpl);

static int cvInitialSetup(CVodeMem cv_mem);

static booleantype cvAllocVectors(CVodeMem cv_mem, FMIC_N_Vector tmpl);
static void cvFreeVectors(CVodeMem cv_mem);

static int cvEwtSetSS(CVodeMem cv_mem, FMIC_N_Vector ycur, FMIC_N_Vector weight);
static int cvEwtSetSV(CVodeMem cv_mem, FMIC_N_Vector ycur, FMIC_N_Vector weight);
...
int CVodeInit(void *cvode_mem, CVRhsFn f, realtype t0, FMIC_N_Vector y0)
{
  CVodeMem cv_mem;
  ...
  /* Set space requirements for one FMIC_N_Vector */

  if (y0->ops->nvspace != NULL) {
    FMIC_N_VSpace(y0, &lrw1, &liw1);
  } else {
    lrw1 = 0;
    liw1 = 0;
  }
  ...
  FMIC_N_VScale(ONE, y0, cv_mem->cv_zn[0]);
  ...
  for (i=0; i<SUNDIALS_TIMER_COUNT; ++i)
    TimerSumReset(i);
...
int CVodeReInit(void *cvode_mem, realtype t0, FMIC_N_Vector y0)
{
  CVodeMem cv_mem;
  int i,k;
...
int CVodeSVtolerances(void *cvode_mem, realtype reltol, FMIC_N_Vector abstol)
{
  CVodeMem cv_mem;
  ...
  if ( !(cv_mem->cv_VabstolMallocDone) ) {
    cv_mem->cv_Vabstol = FMIC_N_VClone(cv_mem->cv_ewt);
    lrw += lrw1;
    liw += liw1;
  ...
  cv_mem->cv_reltol = reltol;
  FMIC_N_VScale(ONE, abstol, cv_mem->cv_Vabstol);
...
int CVode(void *cvode_mem, realtype tout, FMIC_N_Vector yout,
...
int CVodeGetDky(void *cvode_mem, realtype t, int k, FMIC_N_Vector dky)
{
  realtype s, c, r;
  ...
  for (j=q; j >= k; j--) {
    ...
    if (j == q) {
      FMIC_N_VScale(c, zn[q], dky);
    } else {
      FMIC_N_VLinearSum(c, zn[j], s, dky, dky);
    }
  }
  if (k == 0) return(CV_SUCCESS);
  r = SUNRpowerI(h,-k);
  FMIC_N_VScale(r, dky, dky);
  return(CV_SUCCESS);
}
...
static booleantype cvAllocVectors(CVodeMem cv_mem, FMIC_N_Vector tmpl)
{
  ...
  ewt = FMIC_N_VClone(tmpl);
  if (ewt == NULL) return(FALSE);

  acor = FMIC_N_VClone(tmpl);
  if (acor == NULL) {
    FMIC_N_VDestroy(ewt);
    return(FALSE);
  }

  tempv = FMIC_N_VClone(tmpl);
  if (tempv == NULL) {
    FMIC_N_VDestroy(ewt);
    FMIC_N_VDestroy(acor);
    return(FALSE);
  }

  ftemp = FMIC_N_VClone(tmpl);
  if (ftemp == NULL) {
    FMIC_N_VDestroy(tempv);
    FMIC_N_VDestroy(ewt);
    FMIC_N_VDestroy(acor);
    return(FALSE);
  }
  ...
  for (j=0; j <= qmax; j++) {
    zn[j] = FMIC_N_VClone(tmpl);
    if (zn[j] == NULL) {
      FMIC_N_VDestroy(ewt);
      FMIC_N_VDestroy(acor);
      FMIC_N_VDestroy(tempv);
      FMIC_N_VDestroy(ftemp);
      for (i=0; i < j; i++) FMIC_N_VDestroy(zn[i]);
      return(FALSE);
    }
  }
...
static void cvFreeVectors(CVodeMem cv_mem)
{
  ...
  FMIC_N_VDestroy(tempv);
  FMIC_N_VDestroy(ftemp);
  for (j=0; j <= maxord; j++) FMIC_N_VDestroy(zn[j]);

  lrw -= (maxord + 5)*lrw1;
  liw -= (maxord + 5)*liw1;

  if (cv_mem->cv_VabstolMallocDone) {
    FMIC_N_VDestroy(Vabstol);
    lrw -= lrw1;
    liw -= liw1;
  }
}
...
static realtype cvUpperBoundH0(CVodeMem cv_mem, realtype tdist)
{
  ...
  FMIC_N_VAbs(zn[0], temp2);
  efun(zn[0], temp1, e_data);
  FMIC_N_VInv(temp1, temp1);
  FMIC_N_VLinearSum(HUB_FACTOR, temp2, ONE, temp1, temp1);

  FMIC_N_VAbs(zn[1], temp2);

  FMIC_N_VDiv(temp2, temp1, temp1);
  hub_inv = FMIC_N_VMaxNorm(temp1);
...
static int cvYddNorm(CVodeMem cv_mem, realtype hg, realtype *yddnrm)
{
  ...
  FMIC_N_VLinearSum(hg, zn[1], ONE, zn[0], y);
  retval = f(tn+hg, y, tempv, user_data);
  nfe++;
  if (retval < 0) return(CV_RHSFUNC_FAIL);
  if (retval > 0) return(RHSFUNC_RECVR);

  FMIC_N_VLinearSum(ONE, tempv, -ONE, zn[1], tempv);
  FMIC_N_VScale(ONE/hg, tempv, tempv);

  *yddnrm = FMIC_N_VWrmsNorm(tempv, ewt);
...
static void cvAdjustAdams(CVodeMem cv_mem, int deltaq)
{
  ...
  for (j=2; j < q; j++)
    FMIC_N_VLinearSum(-l[j], zn[q], ONE, zn[j], zn[j]);
}
...
static void cvIncreaseBDF(CVodeMem cv_mem)
{
  ...
  FMIC_N_VScale(A1, zn[indx_acor], zn[L]);
  for (j=2; j <= q; j++)
    FMIC_N_VLinearSum(l[j], zn[L], ONE, zn[j], zn[j]);
}
...
static void cvRescale(CVodeMem cv_mem)
{
  ...
  factor = eta;
  for (j=1; j <= q; j++) {
    FMIC_N_VScale(factor, zn[j], zn[j]);
    factor *= eta;
  }
  h = hscale * eta;
  next_h = h;
...
static void cvPredict(CVodeMem cv_mem)
{
  ...
  for (k = 1; k <= q; k++)
    for (j = q; j >= k; j--) 
      FMIC_N_VLinearSum(ONE, zn[j-1], ONE, zn[j], zn[j-1]);
}
...
static int cvNlsFunctional(CVodeMem cv_mem)
{
  ...
  loop {
    ...
    FMIC_N_VLinearSum(h, tempv, -ONE, zn[1], tempv);
    FMIC_N_VScale(rl1, tempv, tempv);
    FMIC_N_VLinearSum(ONE, zn[0], ONE, tempv, y);
    /* Get WRMS norm of current correction to use in convergence test */
    FMIC_N_VLinearSum(ONE, tempv, -ONE, acor, acor);
    del = FMIC_N_VWrmsNorm(acor, ewt);
    FMIC_N_VScale(ONE, tempv, acor);
    ...
    if (dcon <= ONE) {
      acnrm = (m == 0) ? del : FMIC_N_VWrmsNorm(acor, ewt);
...
static int cvNlsNewton(CVodeMem cv_mem, int nflag)
{
  FMIC_N_Vector vtemp1, vtemp2, vtemp3;
  ...
  /* Output of predicted solution in zn[0] */
#ifdef SUNDIALS_DUMP_ITER_STATS
    sprintf(fname, "cvlog_%03d_newton_ypred.txt", nst, nfe);
...
static int cvNewtonIteration(CVodeMem cv_mem)
{
  ...
  loop {
    ...
    FMIC_N_VLinearSum(rl1, zn[1], ONE, acor, tempv);
    FMIC_N_VLinearSum(gamma, ftemp, -ONE, tempv, tempv);

#ifdef SUNDIALS_DUMP_ITER_STATS
    sprintf(fname, "cvlog_%03d_%03d_newton_y_yn_fy_res.txt", nst, nfe);
    dumpfile = fopen(fname, "w");
    n = NV_LENGTH_S(ftemp);
    for (i=0; i<n; ++i) {
        fprintf(dumpfile, "%15.2f %10.15g  %10.15g %10.15g\n", NV_DATA_S(zn[0])[i], NV_DATA_S(zn[1])[i], NV_DATA_S(ftemp)[i], NV_DATA_S(tempv)[i]);
    }
    fclose(dumpfile);
#endif
...
#ifdef SUNDIALS_DUMP_ITER_STATS
    sprintf(fname, "cvlog_%03d_%03d_newton_b.txt", nst, nfe);
    dumpfile = fopen(fname, "w");
    n = NV_LENGTH_S(b);
    for (i=0; i<n; ++i) {
        fprintf(dumpfile, "%10.15g\n", NV_DATA_S(b)[i]);
    ...
    del = FMIC_N_VWrmsNorm(b, ewt);
    FMIC_N_VLinearSum(ONE, acor, ONE, b, acor);
    FMIC_N_VLinearSum(ONE, zn[0], ONE, acor, y);

#ifdef SUNDIALS_DUMP_ITER_STATS
    sprintf(fname, "cvlog_newton_del.txt");
    dumpfile = fopen(fname, "a");
    fprintf(dumpfile, "%03d %03d %10.15g\n", nst, nfe, del);
    fclose(dumpfile);
#endif
...
static void cvCompleteStep(CVodeMem cv_mem)
{
  ...
  for (j=0; j <= q; j++) 
    FMIC_N_VLinearSum(l[j], acor, ONE, zn[j], zn[j]);
  qwait--;
  if ((qwait == 1) && (q != qmax)) {
    FMIC_N_VScale(ONE, acor, zn[qmax]);
    saved_tq5 = tq[5];
    indx_acor = qmax;
  }
}
...
static realtype cvComputeEtaqm1(CVodeMem cv_mem)
{
  ...
  etaqm1 = ZERO;
  if (q > 1) {
    ddn = FMIC_N_VWrmsNorm(zn[q], ewt) * tq[1];
...
static realtype cvComputeEtaqp1(CVodeMem cv_mem)
{
  ...
  if (q != qmax) {
    ...
    FMIC_N_VLinearSum(-cquot, zn[qmax], ONE, acor, tempv);
    dup = FMIC_N_VWrmsNorm(tempv, ewt) * tq[3];
...
static int cvHandleFailure(CVodeMem cv_mem, int flag)
{
  ...
  /*
  FMIC_N_VProd(acor, ewt, tempv);
  FMIC_N_VAbs(tempv, tempv);
  */
...
static void cvBDFStab(CVodeMem cv_mem)
{
  ...
  if (q >= 3) {
    ...
    sqm1 = factorial*q*FMIC_N_VWrmsNorm(zn[q], ewt);
    sqm2 = factorial*FMIC_N_VWrmsNorm(zn[q-1], ewt);
    ssdat[1][1] = sqm2*sqm2;
    ssdat[1][2] = sqm1*sqm1;
    ssdat[1][3] = sq*sq;
  }  
...
static int cvRcheck1(CVodeMem cv_mem)
{
  ...
  smallh = hratio*h;
  tplus = tlo + smallh;
  FMIC_N_VLinearSum(ONE, zn[0], hratio, zn[1], y);
...
static int cvRcheck2(CVodeMem cv_mem)
{
  ...
  if ( (tplus - tn)*h >= ZERO) {
    hratio = smallh/h;
    FMIC_N_VLinearSum(ONE, y, hratio, zn[1], y);
  } else {
...
int cvEwtSet(FMIC_N_Vector ycur, FMIC_N_Vector weight, void *data)
{
  CVodeMem cv_mem;
  int flag = 0;
...
static int cvEwtSetSS(CVodeMem cv_mem, FMIC_N_Vector ycur, FMIC_N_Vector weight)
{
  FMIC_N_VAbs(ycur, tempv);
  FMIC_N_VScale(reltol, tempv, tempv);
  FMIC_N_VAddConst(tempv, Sabstol, tempv);
  if (FMIC_N_VMin(tempv) <= ZERO) return(-1);
  FMIC_N_VInv(tempv, weight);
  return(0);
}
...
{
  FMIC_N_VAbs(ycur, tempv);
  FMIC_N_VLinearSum(reltol, tempv, ONE, Vabstol, tempv);
  if (FMIC_N_VMin(tempv) <= ZERO) return(-1);
  FMIC_N_VInv(tempv, weight);
  return(0);
}
